...
Remember, the Keycloak application for the `Identity and Access Manegement` management isn't configured to `scale to zero`, because the application is `stateful` and contains the realm configuration, which will be delete, if you would restart the application.
